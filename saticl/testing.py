...
from saticl.prepare import prepare_metrics, prepare_model
...
def test(test_config: TestConfiguration):
    ...
    if test_config.store_predictions:
        store_indices = np.random.choice(len(data), size=test_config.pred_count, replace=False)
        for i, (images, y_trues, y_preds) in tqdm(enumerate(data)):
            if i not in store_indices:
                continue
